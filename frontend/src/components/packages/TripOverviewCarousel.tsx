  ...
  return (
    <div className="py-8 md:py-12">
      ...
      <div className="relative">
        <button
          onClick={() => scroll('left')}
          className="absolute left-4 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-white/90 backdrop-blur-sm shadow-lg border-2 border-teal-200 flex items-center justify-center text-gray-800 hover:bg-teal-500 hover:text-white hover:scale-110 transition-all duration-300"
        >
        ...
        <button
          ...
          className="absolute right-4 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-white/90 backdrop-blur-sm shadow-lg border-2 border-teal-200 flex items-center justify-center text-gray-800 hover:bg-teal-500 hover:text-white hover:scale-110 transition-all duration-300"
        >
        ...
        <div
          ref={carouselRef}
          className="flex gap-6 overflow-x-auto scroll-smooth snap-x snap-mandatory px-8 md:px-16 pb-4 scrollbar-hide"
          style={{ scrollbarWidth: 'none' }}
        >
          {packages.map((pkg, index) => (
            <div
              key={`${pkg.id}-${index}`}
              className="relative flex-shrink-0 w-[300px] group cursor-pointer snap-center"
            >
              ...
              <div className="relative h-[400px] rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500">
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-gray-900 via-gray-900/95 to-transparent text-white p-6 transform translate-y-[calc(100%-4rem)] group-hover:translate-y-0 transition-transform duration-500">
